...
managerMsgFuncListData = toList $ do
    ...
    "clientIsDisconnected"  +: [["NodeId"], ["C.Chan", "MsgToSender"]]
    "serverIsDead"          +: []
    "findBestConnects"      +: []
    "testSendMessage"       +: [["NodeId"]]
    "datagramMsg"           +: [["B.ByteString"], ["NodeId"]]
    "initDatagram"          +: [["C.Chan", "MsgToSender"], ["HostAddress"], ["B.ByteString"]]
    "connectivityQuery"     +: []
    "infoRequest"           +: []
    "queryPositions"        +: []
...
nodeBaseDataList = toList $ do
    ...
    "ppNodes"           !: ["M.Map", "PPId", "PPNode"]
    "exitChan"          !: ["C.Chan", "ExitMsg"]
    "bootNodes"         !: ["BootNodeList"]
    "answerChan"        !: ["C.Chan", "Answer"]
    "broadcastNum"      !: ["Int"]
