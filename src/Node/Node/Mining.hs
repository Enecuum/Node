...
miningNodeAnswerClientIsDisconnected aMd
    (toManagerMsg -> ClientIsDisconnected aNodeId aChan) = do
        ...
        whenJust (aNodeId `M.lookup`(aData^.nodes)) $ \aNode -> do
            writeLog (aData^.infoMsgChan)  $ "The node " ++ show aNodeId ++ " is disconnected."
...
answerToShardingNodeRequestMsg aMd
    (toManagerMsg -> ShardingNodeRequestMsg aNetLvlMsg) = do
        ...
        let aLogAboutAliveRequest aNodeId =
                writeLog (aData^.infoMsgChan)  $ aLogMsg "is this neighbor alive" ++
                    "Noda about which they asked: " ++  show aNodeId
        case aNetLvlMsg of
            T.NewPosiotionMsg aMyNodePosition -> do
                writeLog (aData^.infoMsgChan)  $ aLogMsg "new position"
                    ++ ". The position is "
            ...
            T.IamAwakeRequst _ aMyNodePosition -> do
                writeLog (aData^.infoMsgChan)  $ aLogMsg "awake logic lvl"
                sendBroadcast aMd
                    (BroadcastLogic $ BroadcastPosition
                        (aData^.myNodeId)
                        (toNodePosition aMyNodePosition))
            ...
            T.ShardListRequest shardHashes -> do
                writeLog (aData^.infoMsgChan)  $ aLogMsg "needed shards"
                writeLog (aData^.infoMsgChan)  $ "The list of requested shards:" ++ show shardHashes
            ...
            T.IsTheNeighborAliveRequest aNodeId aNodePosition
                | aData^.iAmBroadcast -> if
                    | Just _ <- aData^.nodes.at aNodeId -> do
                        ...
                        writeLog (aData^.infoMsgChan)  $ "The node is alive."
                        return ()
                    | otherwise -> do
                        aLogAboutAliveRequest aNodeId
                        writeLog (aData^.infoMsgChan)  $ "The node is dead."
                ...
                | otherwise -> do
                    ...
                    aLogAboutAliveRequest aNodeId
                    writeLog (aData^.infoMsgChan)  $ "Request to broadcast node about state (alive or dead) of " ++ show aNodeId
...
answerToDeleteOldestMsg aMd _ = do
    aData <- readIORef aMd
    writeLog (aData^.infoMsgChan)  "Cleaning of index of bradcasted msg."
...
instance BroadcastAction ManagerNodeData where
    makeBroadcastAction _ aMd _ aBroadcastSignature aBroadcastThing = do
        aData <- readIORef aMd
        writeLog (aData^.infoMsgChan)  $ "Recived the broadcast msg " ++ show aBroadcastThing ++ "."
...
instance PackageTraceRoutingAction ManagerNodeData ResponcePackage where
    makeAction aChan md aNodeId aTraceRouting aResponcePackage = do
        aData <- readIORef md
        writeLog (aData^.infoMsgChan)  $ "Recived a responce package."
        when (verify (aTraceRouting, aResponcePackage)) $ if
            | isItMyResponce aNodeId aTraceRouting  -> do
                writeLog (aData^.infoMsgChan)  $ "The responce is for me. The processing of responce."
                aProcessingOfAction
      ...
      where
        ...
        aSendToNeighbor aData = do
            writeLog (aData^.infoMsgChan)  $ "This is someone else's message. Resending of responce."
...
answerToNewTransaction aMd (NewTransaction aTransaction) = do
    aData <- readIORef aMd
    writeLog (aData^.infoMsgChan)  $ "I create a transaction: " ++ show aTransaction
...
answerToBlockMadeMsg aMd (toManagerMiningMsg -> BlockMadeMsg aMicroblock) = do
    ...
    writeLog (aData^.infoMsgChan)  $ "I create a a microblock: " ++ show aMicroblock
...
processingOfBroadcastThing aMd aBroadcastThing = do
    aData <- readIORef aMd
    writeLog (aData^.infoMsgChan)  $ "Recived " ++ show aBroadcastThing
