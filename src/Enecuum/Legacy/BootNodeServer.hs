...
bootNodeServer aReceivePort aInfoChan aFileServerChan = do
    writeLog aInfoChan [ServerBootNodeTag, InitTag] Info $ "Init. ServerPoABootNode: a port is " ++ show aReceivePort
            ...
            Right a -> case a of
                ...
                RequestPotentialConnects aFull
                    | aFull -> do
                        ...
                        aConnects <- getRecords aFileServerChan
                        aSend $ ResponsePotentialConnects $ filter (\(Connect aAdress _) -> aHostAdress /= aAdress)
                                                                   aConnects
                    | otherwise -> do
                ...
                ActionConnectIsDead aDeadConnect -> void $ tryWriteChan aInChan $ TestExistedConnect aDeadConnect

                _ -> writeLog aInfoChan [ServerBootNodeTag] Warning $ "Broken message from PP " ++ show aMsg
            Left a -> do
