...
routerActorStart
    :: InChan SyncEvent -> (InChan MsgToCentralActor, OutChan MsgToCentralActor) -> IORef NetworkNodeData -> IO ()
routerActorStart aSyncChan (_, aOutChan) aMd = do
    ...
    undead (aWriteLog [NetLvlTag] Warning "networkNodeStart. This node could be die!") $ forever $ do
        aData <- readIORef aMd
        readChan aOutChan >>= \case
            NodeIsDisconnected aNodeId -> whenJust (aData ^. connects . at aNodeId) $ \_ -> do
            ...
            MsgFromNode aNodeType aMsgFromNode -> do
                void $ C.forkIO $ when (aNodeType /= NN) $ forM_ (aData ^. connects) $ \aNode ->
                    when (aNode ^. nodeType == NN) $ void $ C.forkIO $ writeInChan (aNode ^. nodeChan) aMsgFromNode

                case aMsgFromNode of
                    aMsg@(MsgBroadcast _ aRecieverType _) -> do
                        aNetLog $ "Resending. Broadcast."
                        forM_ (aData ^. connects) $ \aNode ->
                            when
                                    (  aNode
                                    ^. nodeType
                                    /= NN
                                    && (aRecieverType == aNode ^. nodeType || aRecieverType == All)
                                    )
                                $ void
                                $ tryWriteChan (aNode ^. nodeChan) aMsg
                    ...
                    aMsg@(MsgMsgTo (IdFrom aSender) (IdTo aId) aContent) -> do
                        ...
                        if aId == toNodeId (aData ^. nodeConfig . myNodeId)
                            ...
                            else whenJust (aData ^. connects . at aId)
                                $ \aNode -> void $ tryWriteChan (aNode ^. nodeChan) aMsg
                    ...
                    aMsg@(MsgMicroblock aMicroblock) -> do
                        ...
                        void $ C.forkIO $ forM_ (aData ^. connects) $ \aNode ->
                            when (aNode ^. nodeType == All) $ writeInChan (aNode ^. nodeChan) aMsg
                    ...
                    aMsg@(MsgKeyBlock aKeyBlock) -> do
                        ...
                        void $ C.forkIO $ forM_ (aData ^. connects) $ \aNode -> when (aNode ^. nodeType /= NN) $ do
            ...
            ActionFromNode aMsgFromNode -> do
                case aMsgFromNode of
                    NewConnect aNodeId aNodeType aChan aConnect -> do
                        aNetLog
                            $  "A new connect with PP node "
                            ++ show aNodeId
                            ++ ", the type of node is "
                            ++ show aNodeType
                        when (isNothing $ aData ^. connects . at aNodeId) $ modifyIORef aMd $ connects %~ M.insert
                            aNodeId
                            (NodeInfo aChan aNodeType aConnect)
                    RequestListOfPoW (IdFrom aNodeId) -> whenJust (aData ^. connects . at aNodeId) $ \aNode -> do
...
toActualConnectInfo (aNodeId, NodeInfo _ aNodeType aConnect) = ActualConnectInfo aNodeId aNodeType aConnect
