...
runClient
    ...
    -> IO a
runClient host port path = runClientWith host port path defaultConnectionOptions []
...
runClientWith host port path0 opts customHeaders app = do
    ...
    onException
        (S.connect sock (S.addrAddress addr) >> runClientWithSocket sock fullHost path opts customHeaders app)
