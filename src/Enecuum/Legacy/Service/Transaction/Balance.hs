...
getBalanceOfKeys db isStorno tx = do
    ...
    let fun3 (k, v) = case v of
            Nothing       -> (k, initialMoney)
...
-- FIX: Write to db atomically
    ...
    let goOn = macroblockIsOk macroblock where macroblockIsOk MacroblockBD {..} = length _mblocks <= length _teamKeys
    ...
    when goOn $ do
        ...
        when (isMacroblockNew && isMicroblockNew) $ do
            ...
            when (aIsMacroblockClosed && branch == Main)
                $ calculateLedger c False (_keyBlock (m :: Microblock)) macroblock
...
writeMicroblockDB c@(Common descr i) m = do
    ...
    writeLog i [BDTag] Info
        $  "Going to add microblock hashOfMicroblock "
        ++ show hashOfMicroblock
        ++ "to key block "
        ++ show hashKeyBlock
            ...
            case hashOfDBKeyBlock of
                ...
                Just h  -> if h == prev_hash
                    then updateMacroblockByKeyBlock c aKeyBlockHash aKeyBlock Main
                        ...
                        let mes =
                                "Hashes doesn't much: current hash: " ++ show h ++ "previous hash: " ++ show prev_hash
                        writeLog i [BDTag] Info mes
