...
import           Enecuum.Legacy.Refact.Crypto.SerializeInstances   (roll,
                                                                    unroll)
...
getTxs c mb = do
  ...
  where
    getTx1 :: Common -> HashOfTransaction -> IO TransactionInfo
...
tKBIPoW2KBI KeyBlockInfoPoW {..} = KeyBlockInfo {_time , _prev_hash , _number , _nonce , _solver = pubKey, _type }
    where pubKey = publicKey256k1 ((roll $ B.unpack _solver) :: Integer)


fillMacroblockByKeyBlock :: MacroblockBD -> KeyBlockInfo -> MacroblockBD
fillMacroblockByKeyBlock m KeyBlockInfo {..} =
    m { _prevHKBlock = Just _prev_hash, _solver = _solver, _time = _time, _number = _number, _nonce = _nonce }


tMicroblock2MicroblockBD :: Microblock -> MicroblockBD
tMicroblock2MicroblockBD Microblock {..} =
    MicroblockBD {_keyBlock , _signBD = _sign, _publisher , _transactionsHashes = map rHashT _transactions}
...
tMacroblock2KeyBlockInfo MacroblockBD {..} = KeyBlockInfo
  ...
  where
    prev_hash = case _prevHKBlock of
        Nothing -> ""
        Just j  -> j
...
tMacroblock2MacroblockAPI c MacroblockBD {..} = do
    ...
    let microblocksInfoAPI = map
            (\(h, MicroblockBD {..}) -> MicroblockInfoAPI
                { _prevMicroblock = Nothing
                , _nextMicroblock = Nothing
                , _keyBlock
                , _signAPI        = _signBD
