...
decodeTransactionAndFilterByKey pubKey rawTx = txAPI
  where
    txInfo = decodeThis "TransactionInfo" rawTx :: TransactionInfo
    tx     = _tx (txInfo :: TransactionInfo)
    txAPI  = if txFilterByKey pubKey tx then Just TransactionAPI {_tx = tx, _txHash = rHashT tx} else Nothing
