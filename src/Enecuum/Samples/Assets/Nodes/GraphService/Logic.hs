...
module Enecuum.Samples.Assets.Nodes.GraphService.Logic where
...
-- | DumpToDB command.
handleDumpToDB :: GraphServiceData -> DumpToDB -> L.NodeL (Either Text D.SuccessMsg)
handleDumpToDB nodeData _ = do
    L.writeVarIO (nodeData ^. dumpToDBSignal) True
    pure $ Right D.SuccessMsg

-- | RestoreFromDB command.
handleRestoreFromDB :: GraphServiceData -> RestoreFromDB -> L.NodeL (Either Text D.SuccessMsg)
handleRestoreFromDB nodeData _ = do
    L.writeVarIO (nodeData ^. restoreFromDBSignal) True
    pure $ Right D.SuccessMsg

-- | Accept transaction
acceptTransaction :: GraphServiceData -> CreateTransaction -> L.NodeL (Either Text D.SuccessMsg)
