...
networkWorker control registry = go 0
  ...
  act _ = do
    ...
    case controlReq of
      T.RelayRpcRequest _ toAddr req -> makeRpcRequestRelay toAddr req
  ...
  makeRpcRequestRelay toAddr req = do
        ...
        controlResp <- case Map.lookup toAddr nodes of
            Nothing -> pure $ T.AsErrorResponse
...
sendRequest' :: T.NodesRegistry -> D.Address -> RpcRequest -> IO (Either Text RpcResponse)
sendRequest' registry toAddr req = findNode registry toAddr >>= \case
  ...
  Just nodeRt -> do
    ...
    case controlResp of
      T.AsRpcResponse rpcResp -> pure $ Right rpcResp
      T.AsErrorResponse err   -> pure $ Left $ "Control error got: " +| err |+ "."
